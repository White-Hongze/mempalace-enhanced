...
def _generate_aaak_bootstrap(
    ...
    """
    Generate entity registry + critical facts bootstrap from onboarding data.
    ...
    # Build entity codes (first 3 letters of name, uppercase) for quick reference
    entity_codes = {}
    for p in people:
        name = p["name"]
        code = name[:3].upper()
    ...
    # Entity registry (plain language)
    registry_lines = [
        "# 实体注册表",
        "# 由 mempalace init 自动生成，可根据需要更新。",
        "",
        "## 人物",
    ]
    for p in people:
...
""")
    ...
    # Generate entity registry + critical facts bootstrap
